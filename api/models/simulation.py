...
class SimulationScenario(Base):
    ...
    scenario_data = Column(Text)  # JSON as string
    instructions = Column(Text)
    is_published = Column(Boolean, default=False)
    created_date = Column(DateTime, default=datetime.utcnow)
    modified_date = Column(DateTime)
    author_id = Column(String)  # Можно добавить ForeignKey('users.id')
    approver_id = Column(String)  # Можно добавить ForeignKey('users.id')
    approval_date = Column(DateTime)
    version = Column(String(50))
    min_score_to_pass = Column(Float)
    is_active = Column(Boolean, default=True)

    # Relationships (без внешних моделей)
    theme_tasks = relationship("SimulationScenarioThemeTask", back_populates="scenario", cascade="all, delete-orphan")
    results = relationship("SimulationResult", back_populates="scenario", cascade="all, delete-orphan")
    checkpoints = relationship("ScenarioCheckpoint", back_populates="scenario", cascade="all, delete-orphan")
...
class SimulationScenarioThemeTask(Base):
    ...
    theme_task_id = Column(String, primary_key=True)  # Можно добавить ForeignKey('theme_tasks.id')

    scenario = relationship("SimulationScenario", back_populates="theme_tasks")
    # theme_task = relationship("ThemeTask", back_populates="simulation_scenarios")  # пока исключено
...
class SimulationResult(Base):
    ...
    id = Column(String, primary_key=True, default=generate_uuid)
    user_id = Column(String)  # Можно добавить ForeignKey('users.id')
    ...
    checkpoint_results = relationship("CheckpointResult", back_populates="simulation_result", cascade="all, delete-orphan")
...
class ScenarioCheckpoint(Base):
    ...
    checkpoint_results = relationship("CheckpointResult", back_populates="checkpoint", cascade="all, delete-orphan")
...
# SQLite (для локальной быстрой проверки)
